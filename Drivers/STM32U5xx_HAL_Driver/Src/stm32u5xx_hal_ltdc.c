...
HAL_StatusTypeDef HAL_LTDC_DeInit(LTDC_HandleTypeDef *hltdc)
{
  ...
  while (MV_READ_BIT(hltdc->Instance->CDSR, LTDC_CDSR_VSYNCS) == 0U)
  {
...
{
  uint32_t isrflags  = MV_READ_REG(hltdc->Instance->ISR);
  uint32_t itsources = MV_READ_REG(hltdc->Instance->IER);
